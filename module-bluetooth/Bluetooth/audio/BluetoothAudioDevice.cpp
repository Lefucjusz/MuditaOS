...
auto BluetoothAudioDevice::scaleVolume(audio::Stream::Span &dataSpan) -> void
{
    ...
    constexpr auto volumeScaleLut = utils::makeArray<float, volumeLevels>([](auto index){
         ...
         * For 10 available steps this gives ~4dB change between each step, which is a bit
         * too much, but it seems to be a fair compromise between step size and minimum
         * volume. The best idea would be to increase number of available steps, as having
         * only ten is not enough for convenient volume control.
         * Either way, this solution is much better than previous one with linear scale...
...
audio::AudioDevice::RetCode A2DPAudioDevice::waitUntilStreamPaused()
{
