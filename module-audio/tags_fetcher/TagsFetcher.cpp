    ...
    namespace
    {
        ...
        std::optional<Tags> fetchTagsInternal(const std::string &filePath)
        {
            ...
            if (!tagReader.isNull() && (tags != nullptr)) {
                ...
                const std::uint32_t duration_min     = total_duration_s / utils::time::secondsInMinute;
                const std::uint32_t duration_hour    = duration_min / utils::time::secondsInMinute;
                const std::uint32_t duration_sec     = total_duration_s % utils::time::secondsInMinute;
